...
const app = express();
const port = 3000;
const walletAddress = wallet_address  // Use the wallet address from .env
const connection = new Connection(solana_rpc_url, 'confirmed');  // Use the RPC URL from .env
...
const getTransactions = async (address, numTx) => {
  ...
  const detailedTransactions = await Promise.all(transactionList.map(async (transaction, index) => {
    try {
      ...
      return {
        index: index + 1, // Add index to the transaction
